...
from seahub.pro.seafobj import fs_mgr
    ...
    def diff(self):
            ...
            for dent in dir1.get_files_list():
                ...
                if not new_dent or new_dent.type != dent.type:
                    ...
                    if new_dent.id == dent.id:
                        ...
                        modified_files.append((make_path(path, dent.name), new_dent.id))

            added_files.extend([(make_path(path, dent.name), dent.id) for dent in dir2.get_files_list()])
        ...
        while True:
            ...
            try:
                ...
                added_dirs.append((path, obj_id))
            except IndexError:
                break
            d = fs_mgr.load_seafdir(self.repo_id, self.version, obj_id)
            added_files.extend([(make_path(path, dent.name), dent.id) for dent in d.get_files_list()])
